...
<root
    ...
    void RendererDevice::updateDeviceInfo(const std::string& name, const std::string& uuid,
    ...
    void RendererDevice::setDeviceInfo(const std::string& key, const std::string& value) {
        if (key.empty()) return;
    ...
    void RendererDevice::addServiceXML(const std::string& name, int version) {
        ...
        snprintf(buf, sizeof(buf), "urn:schemas-upnp-org:service:%s:%d", name.c_str(), version);
        e1->SetText(buf);
    ...
    std::unordered_map<std::string, std::shared_ptr<RendererService>>& RendererDevice::getServiceList() {
        return serviceMap;
    }

    std::string RendererDevice::parseRequest(const std::string& service, const std::string& action, const std::string& data) {
        if (serviceMap.count(service) == 0) return RendererService::dummyRequest(service, action);
    ...
    inline void parsePostAction(mg_http_message* hm, std::string& service, std::string& action) {
        size_t i, max = sizeof(hm->headers) / sizeof(hm->headers[0]);
        for (i = 0; i < max && hm->headers[i].name.len > 0; i++) {
            struct mg_str *k = &hm->headers[i].name, *v = &hm->headers[i].value;
            if (mg_vcasecmp(k, "SOAPACTION") == 0) {
                ...
                for(size_t index = 0; index < v->len; index++) {
                    if (v->ptr[index] == '#' && index + 1 < v->len) {
                        action = std::string{v->ptr + index + 1, v->len - index - 2};
                        break;
                    }
                    if (v->ptr[index] == ':' ) {
                        count++;
                        if (count == 4) {
                            service = std::string{v->ptr + last + 1, index - last - 1};
                        }
                        last = index;
                    }
                }
        ...
        if (ev == MG_EV_HTTP_MSG) {
            ...
            if (mg_vcasecmp(&hm->method, "GET") == 0) {
                if (mg_strcmp(hm->uri, MG_C_STR("/")) == 0) {
                    ...
                    mg_http_reply(c, 200, XML_HEADER, soap->device.getString().c_str());
                } else {
                ...
                auto res = soap->device.parseRequest(service, action, std::string{hm->body.ptr, hm->body.len});
                if (!res.empty()) {
