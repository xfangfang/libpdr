...
namespace pdr {
    ...
    SSDPService::SSDPService(const std::string uuid,
                             const std::string &scope,
                             const std::string &name,
                             const std::string& location) : uuid(uuid), scope(scope), name(name), location(location){}
    ...
    void SSDP::fn(struct mg_connection *c, int ev, void *ev_data, void *fn_data) {
        ...
        if (ev == MG_EV_READ) {
            ...
            if (mg_http_parse((const char *) c->recv.buf, c->recv.len, &hm) <= 0) return;
            ...
            for(auto& service : ssdp->getServices()) {
                if (service.second.getST() == headers["ST"] || headers["ST"] == "ssdp:all") {
                    mg_printf(c,
                              "HTTP/1.1 200 OK\r\n"
                              "USN: %s\r\n"
                              "ST: %s\r\n"
